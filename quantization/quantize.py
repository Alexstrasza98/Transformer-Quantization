    ...
    for name, layer in model.named_children():
        ...
        if "sublayer_" in name:
            for sub_name, sub_layer in getattr(model, name).named_children():
                # Quantization
                if type(sub_layer) == nn.Linear and quantize_all_linear:
                    model.__dict__["_modules"][sub_name] = QuantizedLinear(
                        sub_layer.in_features, sub_layer.out_features, weight_bits=quantization_bits
                    )
                elif type(sub_layer) == nn.Linear:
                    if sub_name in ["0", "1", "2"]:
                        model.__dict__["_modules"][sub_name] = QuantizedLinear(
                            sub_layer.in_features, sub_layer.out_features, weight_bits=quantization_bits
                        )
                else:
                    layer_types = [type(l) for l in sub_layer.modules()]

                    if nn.Linear in layer_types:
                        quantizer(sub_layer, quantization_bits, quantize_all_linear)
        else:
            # Quantization
